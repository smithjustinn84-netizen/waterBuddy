...
class WaterRepositoryImplTest {
    ...
    @Test
    fun `observeDailyStats combines intakes and goal correctly`() =
        runTest {
            ...
            every { waterIntakeDao.getAllWaterIntakes() } returns
                flowOf(
                    listOf(
                        intake1,
                        intake2,
    ...
    @Test
    fun `observeDailyStats uses default goal when none exists`() =
        runTest {
            val date = LocalDate(2023, 10, 27)
    ...
    @Test
    fun `observeStatsRange generates stats for each day in range`() =
        runTest {
    ...
    @Test
    fun `addWaterIntake calls dao and returns success`() =
        runTest {
    ...
    @Test
    fun `addWaterIntake with default note parameter calls dao and returns success`() =
        runTest {
            everySuspend { waterIntakeDao.insertWaterIntake(any()) } returns Unit
    ...
    @Test
    fun `addWaterIntake returns failure when dao throws`() =
        runTest {
    ...
    @Test
    fun `deleteWaterIntake calls dao and returns success`() =
        runTest {
    ...
    @Test
    fun `updateDailyGoal calls dao and returns success`() =
        runTest {
    ...
    @Test
    fun `getDailyGoal returns value from dao or default`() =
        runTest {
